...
```{r}
# récupère une liste de xml
get_PMID_xml_list <- function(list_PMID) {
datas_PMID <- list()
  for (PMID in list_PMID) {
    rslt_entrez <- entrez_fetch(db = "pubmed", id=PMID, rettype = "xml",
                                  parsed = TRUE)
    datas_PMID <- list(datas_PMID, rslt_entrez)
  }
  return(date_PMID)
}

PMID_testez <- get_PMID_xml_list(PMID_form)
...
date_PMID <- data.frame()
  for (PMID in list_PMID) {
    rslt_entrez <- entrez_fetch(db = "pubmed", id=PMID, rettype = "xml",
                                  parsed = TRUE)
    iter_PMID <-xpathSApply(rslt_entrez, "//MedlineCitation/Article/ArticleDate/Year", xmlValue)
