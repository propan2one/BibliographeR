...
```{r}
# Assess neighborhood
# Problems with balise on some abstract inside the xml (e.g. 30310074), the exemple is working but 3 edges instead of 14..
...
abstract <- xml_neighborhood_listed %>%
  map(c("MedlineCitation", "Article", "Abstract", "AbstractText"),.default = NA)
neighborhood_results <-tibble(PMID=PMID_Voisin, TITLE= unlist(TITLE, use.names = FALSE), abstract= unlist(abstract, use.names = FALSE))
...
# tokenize text at the single word (aka unigram) level
neighborhood_results_hn_word_tokens <- neighborhood_results %>% unnest_tokens(word, token = "words", format = "xml", input = abstract)

# remove stop words (e.g. 'a', 'the', 'and')
neighborhood_results_hn_word_tokens_no_stop <- neighborhood_results_hn_word_tokens %>% anti_join(get_stopwords())
# create word counts
neighborhood_results_hn_word_counts <- neighborhood_results_hn_word_tokens_no_stop %>% count(word, sort = T)


# construct ggplot
neighborhood_results_hn_word_counts %>% top_n(20, wt = n) %>%
