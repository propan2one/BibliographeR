...
plot(delete.vertices(simplify(graph), degree(graph)==1))
...
```{r}
recs <- entrez_fetch(db = "pubmed", id = test_id, rettype = "xml", parsed = TRUE)

raw_abs <- entrez_fetch(db="pubmed", id=test_id, rettype="pubdate")
  extract_from_esummary(raw_abs, "date")
...
date_to_tabgraph<- harvest_PMID_date(PMID_form)

  for (PMID in PMID_form) {
  #PMID <- PMID_form[1]
  get_PMID_Date <- entrez_fetch(db = "pubmed", id=PMID, rettype = "xml",
                                parsed = TRUE)
  iter_PMID <- rbind(iter_PMID, c(PMID, xpathSApply(test_un, "//MedlineCitation/Article/ArticleDate/Year", xmlValue)))
  rm(get_PMID_Date)
  writeLines(PMID)
}

test_un <- entrez_fetch(db = "pubmed", rettype = "xml", parsed = TRUE, web_history=web_histo_tokUn)
xpathSApply(test_un, "//MedlineCitation/Article/ArticleDate/Year", xmlValue)
