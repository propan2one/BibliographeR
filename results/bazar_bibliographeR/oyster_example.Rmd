...
## Tendances de la recherche en rapport aux 10 meilleurs mots-clés
...
Dans ce qui suit, on a été obligé de faire une boucle sale, car sans boucles ça crashait car trop de données à allouer, et avec une boucle en itérant sur 1 à la fois mais sans Sys.sleep, on faisait plus de 3 requêtes par seconde sur l'API entrez et du coup ça crashait aussi...
...
PMID_Voisin <- names(unlist(neighborhood(graph = graph,nodes = "30310074"), use.names = TRUE))
PMID_Voisin <- as.numeric(PMID_Voisin)
entrez_search(db="PMID_Voisin", term=PMID_Voisin, use_history=TRUE)
for (pmid in PMID_Voisin) {

}
```

```{r}
# Recherche en cours sur les voisinages 
PMID_Voisin <- neighborhood(graph = graph,nodes = "30310074") %>% 
...
xml_neighborhood_listed <- xmlToList(xml_neighborhood)

TITLE <- xml_neighborhood_listed %>%
  map(c("MedlineCitation", "Article", "ArticleTitle"),.default = NA)

xpathSApply(xml_neighborhood, "//MedlineCitation/Article/ArticleTitle", xmlValue)
xpathSApply(xml_neighborhood, "//MedlineCitation/Article/Abstract/AbstractText",resolveNamespaces = FALSE,  xmlValue)

ABSTRACT <- xml_neighborhood_listed %>%
  map(c("MedlineCitation", "Article", "Abstract", "AbstractText"),.default = NA)
tibble(PMID=PMID_Voisin, TITLE= unlist(TITLE, use.names = FALSE))


iter_PMID <-xpathSApply(get_PMID_Date, "//MedlineCitation/Article/ArticleDate/Year", xmlValue)
xml_neighborhood_listed$PubmedArticle$MedlineCitation$Article$Abstract$AbstractText



xml <- xmlInternalTreeParse(xml_neighborhood,useInternalNodes=T)
xml["//MedlineCitation/Article/Abstract"]
# https://stackoverflow.com/questions/23683733/parsing-an-xml-file-in-r-which-has-subnodes-having-all-values-with-the-same-tag

# this is where you start
tags  <- xml["//MedlineCitation/Article/Abstract/AbstractText"]
...
names(unlist(ABSTRACT[4]))

Abstract_vect <- c()
ABSTRACT[4]
for (elements in ABSTRACT) {
  if ("PubmedArticle..attrs.Label" %in% names(unlist(elements))) {
    elements$PubmedArticle$text
    Abstract_vect <- append(Abstract_vect, elements$PubmedArticle$text)
  }
  Abstract_vect <- append(Abstract_vect, unlist(elements, use.names = FALSE))
}
ABSTRACT
...
ABSTRACT[[4]] %>% keep("text")

x <- rerun(5, a = rbernoulli(1), b = sample(10))
eaze <- x %>% keep("a")


xml_neighborhood_listed$PubmedArticle$PubmedData$ReferenceList$Reference$ArticleIdList$ArticleId
xml_neighborhood_listed$PubmedArticle$MedlineCitation$Article$Abstract$AbstractText


xml_neighborhood_listed[1]
$PubmedArticle$text
