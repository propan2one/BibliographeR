...
index_df %>%
  filter(Type=="journal") %>%
  count(Title, Issn , year, Type) %>%
  filter(n>1)
# donc faut récupé les Issn de correspondant à chaque articles
...
# Vérification qu'il y a bien le bon nombre d'article
#xml_recherche_ToList %>%
...
index_df <- index_df %>%
  filter(Type=="journal")
index_df <- rename(index_df, title = Title) #For renaming dataframe column
...
# Type <- 
xml_recherche_ToList %>%
  map(c("MedlineCitation", "Article", "Journal"),.default = NA) 
# %>%
#   unlist(use.names = FALSE)

tab <- cbind(tab, Issn) # pas tidy mais c'est pour allé plus vite
...
abstract <- xml_neighborhood_listed %>%
  map(c("MedlineCitation", "Article", "Abstract", "AbstractText"),.default = NA)
dates <- xml_neighborhood_listed %>% # marche bof
